...
func (e *CommandCodeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	if tools, ok := params["tools"].([]any); ok { normalized := make([]any, 0, len(tools)); for _, raw := range tools { if tool, okTool := raw.(map[string]any); okTool { var name, description string; var schema any; if fn, okFn := tool["function"].(map[string]any); okFn { name, _ = fn["name"].(string); description, _ = fn["description"].(string); schema = fn["parameters"] } else { name, _ = tool["name"].(string); description, _ = tool["description"].(string); schema = tool["parameters"] }; if name == "" || schema == nil { continue }; if _, okSchema := schema.(map[string]any); !okSchema { continue }; normalized = append(normalized, map[string]any{"name": name, "description": description, "input_schema": schema}) } }; if len(normalized) == 0 { delete(params, "tools") } else { params["tools"] = normalized } }
