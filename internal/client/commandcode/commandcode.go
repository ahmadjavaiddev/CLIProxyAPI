...
func Headers(apiKey, version, threadID, workingDir string) http.Header { h := make(http.Header); h.Set("Content-Type", "application/json"); h.Set("Accept", "application/json, */*"); h.Set("Authorization", "Bearer "+apiKey); h.Set("User-Agent", "commandcode-cli/"+version+" Node.js/v24.0.0"); h.Set("Accept-Encoding", "gzip, deflate, br"); h.Set("Accept-Language", "en-US,en;q=0.9"); h.Set("x-cli-environment", "production"); h.Set("x-command-code-version", version); h.Set("x-session-id", threadID); h.Set("x-co-flag", "false"); h.Set("x-taste-learning", "false"); h.Set("x-project-slug", ProjectSlug(workingDir)); h.Set("traceparent", Traceparent()); return h }
